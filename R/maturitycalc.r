...
#' @title comparetwosites compares two groups of sites using logistic regression
#' 
#' @description comparetwosites using logistic regressions to compare whether 
...
#'     immature = 0
#' @param siglevel what P level constitutes a significant difference, 
#'     default = 0.05
#' 
#' @seealso {
...
#' }
#'
#' @return a list of site properties, and a summary of the logistic regression
#' @export
#'
#' @examples
#' print("wait on internal data sets")
#' # site1=c(784,459,500);site2=c(499); sitecol="site";samdat=samb;
#' # length="length";mat10="mature";siglevel=0.05
comparetwosites <- function(site1,site2,samdat,sitecol="site",
                            length="length",mat10="mature",siglevel=0.05) { 
  picks <- which(samdat[,sitecol] %in% c(site1,site2))
  sams <- samdat[picks,]
  samsf <- sams
  samsf$sitenum <- NA
  picks1 <- which(samsf$site %in% site1)
  samsf$sitenum[picks1] <- 1
  samsf$sitenum[-picks1] <- site2
  samsf$sitenum <- factor(samsf$sitenum)
  model <- glm(formula = samsf[,mat10] ~ samsf[,length] + samsf$sitenum, 
               family = binomial)
  summ <- summary(model)
  signif <- FALSE
  if (summ$coefficients[3,4] < siglevel) signif <- TRUE
  return(list(summ=summ,signif=signif))
} # end of comparetwosites
...
findgroups <- function(sitenums,samdat,sitecol="site",console=FALSE) {
  ...
  while (count <= nsites) {
    ...
    repeat {
      ...
      ans <- comparetwosites(site1=c(grp),site2=newsite,samdat=samdat,
                             sitecol=sitecol)
      if (ans$signif) break
      grp <- c(grp,newsite)
      count <- count + 1
...
fitgroups <- function(groups,samdat,sitecol="site",length="length",
  ...
  for (i in 1:ng) { #  i=1
    ...
    fits[[i]] <- ans
    param[i,] <- c(ans$param,(ans$out$out$df.null + 1))
  }
...
#'     immature = 0
#'
#' @return a list containin ga matrix of the site parameters and a list of all
...
plotgroups <- function(groups,samdat,xmin=0,xmax=0,sitecol="site",
                       length="length",mat10="mature") { # x = outgs
  outfit <- fitgroups(groups,samdat,sitecol=sitecol,length=length,mat10=mat10)
  fits <- outfit$fits
  nfits <- length(fits)
  parset(plots=pickbound(nfits),byrow=FALSE)
  for (i in 1:nfits) {
...
#'
#' @param sits a vector of two site identification numbers
...
#' @param combine should the combined data set have a maturity logistic fitted
...
plottwosites <- function(sits,samdat,length="length",mat10="mature",
                         sitecol="combsite",combine=TRUE) {  
  if (length(sits) != 2)
    stop("Only select two sites when using plottwosites!  \n")
  ans <- vector("list",length=3)
  for (i in 1:2) {
    picks <- which(samdat[,sitecol] == as.character(sits[i]))
    ans[[i]] <- fitmaturity(samdat[picks,],length=length,mat10=mat10)
  }
  if (combine) {
    picks <- which(samdat[,sitecol] %in% as.character(sits))
    ans[[3]] <- fitmaturity(samdat[picks,],length=length,mat10=mat10)
  }
  ...
  if (combine) {
    ...
    legend("topleft",c(sits[1],sits[2],"combined"),col=c("black","blue",2),
           lwd=3,bty="n",cex=1.2)
  } else {
    legend("topleft",c(sits[1],sits[2]),col=c("black","blue"),lwd=3,bty="n",
           cex=1.2)
  }
  param <- sapply(ans,"[[","param")
  rownames(param) <- colnames(ans[[1]]$param)
  colnames(param) <- c(sits,"combined")
  ans$param <- param
...
#' 
#' @seealso {
#'    \link{getloc}, \link{comparetwosites}
#' } 
#'
