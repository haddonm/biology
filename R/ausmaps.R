...
#' @title getdist extracts the relative distances between sample sites
#' 
#' @description getdist expects maturation data from multiple sites 
#'     that includes latitude and longitude data for the location of
#'     each site. It calculates the approximate crow-flies distance 
#'     between the sites using the getlatlong and heversine functions, 
#'     and then gives the volumetric distance.
#'
#' @param x the data.frame containing the maturity data
#' @param lat the name of the latitude variable
#' @param lon the name of the longitude variable
#' @param site the name of the site variable
#'
#' @return a square matrix containing the approximate distance between
#'     sites in kms
#' @export
#'
#' @examples
...
#' }
getdist <- function(x,lat="lat",lon="long",site="site") {
   #  x=b11; lat="lat";lon="lon";site="site"
   sites <- as.numeric(names(table(x[,site])))
   nsite <- length(sites)
   LL <- getlatlon(x,lat,lon,site)
   dist <- matrix(0,nrow=nsite,ncol=nsite,dimnames=list(sites,sites))
   for (i in 1:nsite) {
      for (j in 1:nsite) {
         tmp <- haversine(LL[i,"Lat"],LL[i,"Long"],
                          LL[j,"Lat"],LL[j,"Long"]) 
         dist[i,j] <- tmp[2] 
      }
   }
...
#' @title getlatlon extracts the latitude and longitudes of sample sites
#' 
#' @description getlatlon expects maturation data from multiple sites 
#'     that includes latitude and longitude data for the location of
#'     each site. It extracts and outputs the lat long for each site.
#'
#' @param x the data.frame containing the maturity data
#' @param lat the name of the latitude variable
#' @param lon the name of the longitude variable
#' @param site the name of the site variable
#'
#' @return a matrix of sites by latitude longitude
#' @export
#'
#' @examples
#' \dontrun{
#'  # wait on a data set being included into the package
#' }
getlatlon <- function(x,lat="lat",lon="long",site="site") {
   #  x=b11; lat="lat";lon="lon";site="site"
   sites <- as.numeric(names(table(x[,site])))
   nsite <- length(sites)
   latlong <- matrix(0,nrow=nsite,ncol=2,
                     dimnames=list(sites,c("Lat","Long")))
   for (i in 1:nsite) {  # i=1; j=2
      pick <- which(x[,site] == sites[i])
      latlong[i,"Lat"] <- x[pick[1],lat]
      latlong[i,"Long"] <- x[pick[1],lon]
   } 
   latlong <- latlong[order(latlong[,2]),]
   return(latlong)
} # end of getdist
...
haversine <- function(lat1,lon1,lat2,lon2) {
   ...
   km <- c * 6371  # equatorial radius = 6378.137 polar radius = 6356.752 
   po <- c * 6356.752 
   eq <- c * 6378.137
   return (c(polar=po,km=km,equat=eq))
} # end of haversine
